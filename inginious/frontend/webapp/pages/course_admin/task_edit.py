...
class CourseEditTask(INGIniousAdminPage):
    ...
    def parse_problem(self, problem_content):
        ...
        if "choices" in problem_content:
            problem_content["choices"] = [val for _, val in sorted(iter(list(problem_content["choices"].items())), key=lambda x: int(x[0]))]
    ...
    def POST_AUTH(self, courseid, taskid):  # pylint: disable=arguments-differ
        ...
        try:
            ...
            data = {key: val for key, val in list(data.items()) if not key.startswith("problem") and not key.startswith("limits")}
            del data["@action"]
            ...
            data["problems"] = OrderedDict([(key, self.parse_problem(val))
                                            for key, val in sorted(iter(list(problems.items())), key=lambda x: int(x[1]['@order']))])
            data["limits"] = limits
