...
def directory_compare_from_hash(from_directory, to_directory):
    ...
    """
    from_directory = dict([(os.path.normpath(path), (filehash, stat)) for path, (filehash, stat) in list(from_directory.items())])
    to_directory = dict([(os.path.normpath(path), (filehash, stat)) for path, (filehash, stat) in list(to_directory.items())])

    to_upload = []
    to_delete = []
    for path, (filehash, stat) in list(from_directory.items()):
